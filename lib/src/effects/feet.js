class Feet {
    ...
    handle(gameData) { 
        ...
        if (gameData.throttle > 0) {
            ...
            this.tactPlay.playEffect('ACCELERATION_E', {intensity:gameData.throttle / 2, duration:this.options['duration']})
        }

        if (gameData.speed > 2 && gameData.speed > this.prevSpeed) {
            let intensity = (gameData.speed - this.speedOnGearShift) / 25
            console.log('ACCELERATION_F', intensity, this.speedOnGearShift)
        ...
        if(this.gear != null && this.gear != gameData.gear && gameData.gear != 0) {
            this.tactPlay.playEffect('CLUTCH_ENGAGE', {intensity:0.5, duration:this.options['duration']})
        ...
        if(gameData.susHeight[0] != 0)
        {
            if(Math.abs(gameData.susHeight[0] - this.prevSus[0]) > diff) {
                ...
                this.tactPlay.playEffect('wheel_shock_absorber_LEFT_FRONT_C', {intensity:0.5, duration:this.options['duration']})
            }            
            
            if(Math.abs(gameData.susHeight[1] - this.prevSus[1]) > diff) {
                this.tactPlay.playEffect('wheel_shock_absorber_RIGHT_FRONT_C', {intensity:0.5, duration:this.options['duration']})
            } 
            
            if(Math.abs(gameData.susHeight[2] - this.prevSus[2]) > diff) {
                this.tactPlay.playEffect('wheel_shock_absorber_RIGHT_BACK_C', {intensity:0.5, duration:this.options['duration']})
            } 
            
            if(Math.abs(gameData.susHeight[3] - this.prevSus[3]) > diff) {
                this.tactPlay.playEffect('wheel_shock_absorber_LEFT_BACK_C', {intensity:0.5, duration:this.options['duration']})
            } 
